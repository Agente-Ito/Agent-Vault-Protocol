...
const INITIAL_TASKS: TaskRecord[] = [
  { id: '1', label: 'Monthly Rent',      description: 'Fixed payment to landlord',  botEmoji: '🏠', botName: 'Rent Bot',          vaultLabel: 'Housing',       nextExecution: new Date(_now.getFullYear(), _now.getMonth(), 28),          intervalLabel: 'Every month',           triggerType: 'timestamp', enabled: true,  amount: 1200, currency: '$' },
  { id: '2', label: 'Spotify Premium',   description: 'Music subscription',          botEmoji: '🎵', botName: 'Subscription Bot',  vaultLabel: 'Entertainment', nextExecution: new Date(_now.getFullYear(), _now.getMonth() + 1, 5),        intervalLabel: 'Every month',           triggerType: 'timestamp', enabled: true,  amount: 11,   currency: '$' },
  { id: '3', label: '60/40 Rebalance',   description: 'DeFi portfolio adjustment',   botEmoji: '📈', botName: 'DeFi Bot',          vaultLabel: 'Investments',   nextExecution: new Date(_now.getFullYear(), _now.getMonth() + 1, 12),       intervalLabel: 'Every 7200 blocks (~1h)',triggerType: 'block',     enabled: true },
];
...
export default function AutomationPage() {
  ...
  const handleToggle  = (taskId: string) => setTasks((prev) => prev.map((t) => t.id === taskId ? { ...t, enabled: !t.enabled } : t));
  const handleNewTask = (task: TaskRecord) => setTasks((prev) => [...prev, task]);
